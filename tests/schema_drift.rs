...
fn test_schema_drift_check() {
  ...
  let schema_path = root.join("formality.schema.json");

  assert!(
    schema_path.exists(),
    "formality.schema.json must exist in repository root. Run 'fml schema -o formality.schema.json' to generate it."
  );

  let committed_schema = fs::read_to_string(&schema_path)
    .expect("Failed to read formality.schema.json");
