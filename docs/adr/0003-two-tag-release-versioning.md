...
The design conversation in `#126 [pre-recreation]` considered decoupling _three_
things onto independent tags: the binary, the VS Code extension, and the schema.
It landed on two, not three: `tests/version_lockstep.rs` already deliberately
enforces `Cargo.toml`'s version matching `editors/vscode/package.json`'s version
— an existing, intentional invariant — and the repo owner confirmed keeping
...
- The extension stays coupled to the binary's `v*` tag — adding a genuinely
  ...
  considered and rejected that shape for now.
