...
export function activate(context: vscode.ExtensionContext) {
  ...
  const onConfigChange = (uri: vscode.Uri) => {
    ...
    if (autoSync) {
      ...
      runFmlCommand(['sync'], workspaceFolder, 'Auto-syncing native configs...');
    }
  };
...
function doFormat(
  ...
  execFile(exe, ['fmt', filePath], { cwd: workspaceRoot }, (error, stdout, stderr) => {
    if (error) {
...
function runFmlCommand(
  ...
  const task = (progress?: vscode.Progress<{ message?: string }>) => {
    ...
    return new Promise<void>((resolve) => {
      execFile(exe, args, { cwd: cwd || process.cwd() }, (error, stdout, stderr) => {
        ...
        if (error) {
          ...
          vscode.window.setStatusBarMessage(`✔ Formality: ${args.join(' ')} complete`, 3000);
        }
